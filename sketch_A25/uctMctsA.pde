int uctMctsABrain(player pl, int expandThreshold, int terminateThreshold, int _depth) { // 
  //候補を一つに絞ってもよいが、いつでも同じ動作になってしまうので、複数個の候補を重みをつけておくとよい。
  //ここから
  startTime=millis();
  int answer = uctMctsAStartingJoseki(pl);
  if (answer!=-1) return answer;
  answer = uctMctsABrainPreparation(pl);
...
int uctMctsAStartingJoseki(player pl) {
  int count=0;
  for (int k=0; k<25; k++) {
...
int uctMctsABrainPreparation(player pl) {
        ...
        for (int p=1; p<5; p++) {
          ...
          do {
            if (nd0.parent!=null) {
              ...
              nd0.na +=bestNode.na;
              for (int pp=1; pp<=4; pp++) {
                nd0.wa[pp] += bestNode.wa[pp];//
                nd0.pa[pp] += bestNode.pa[pp];//
              }
              //println("uctMctsBrain:→　ノード ",nd0.id, "のデータ("+nd0.wa[1]+","+nd0.wa[2]+","+nd0.wa[3]+","+nd0.wa[4]+")/"+nd0.na);
            } else {// ルートまでたどり着いた、の意味
              break;
...
int uctMctsBAttackChance(player pl) {
