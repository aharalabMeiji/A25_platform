...
void showContents() {
  ...
  if (gameOptions.get("gameMode") == 0) {// Game options
    ...
    if (total>0) {
      ...
      buttonDownBoard = new button();
      buttonDownBoard.setLT(int(left+smallBoardDispSize*7), int(top), " ");
      buttonDownBoard.wid=boardDispSize*5;
      buttonDownBoard.hei=boardDispSize*5;
    ...
    if (total>0) {
      ...
      // now board
      buttonDownBoard = new button();
      buttonDownBoard.setLT(int(left+smallBoardDispSize*7), int(top+boardDispSize*5.9), " ");//
      buttonDownBoard.wid=boardDispSize*5;//
      buttonDownBoard.hei=boardDispSize*1.2;//
      simulatorStartBoard.get(now).display(0, int(left+smallBoardDispSize*7), int(top), int(boardDispSize), int(boardDispSize));
      // now text
      fill(0);
      text(now, left+smallBoardDispSize*7, top+boardDispSize*7);
